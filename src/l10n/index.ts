...
export function get<TData, TPath extends string, TDefault = GetFieldType<TData, TPath>>(
    ...
    const value = path.split('.').reduce<GetFieldType<TData, TPath>>((value, key) => (value as any)?.[key], data as any);
...
export const t = (key: PathInto<Resource, string>, opts?: Record<string, string>): string => {
    const result = get(translations, key, '');

    return applyOpts(result, opts);
};

export const plural = (key: PathInto<Resource, Function>, count: number, opts?: Record<string, string>): string => {
...
const currency = (amount: number, currency = 'EUR', options: Intl.NumberFormatOptions = {}) => {
