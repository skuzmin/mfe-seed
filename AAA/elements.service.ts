...
export class ElementsService {
    ...
    public static toggleElementClass(data: string | Element, className: string): void {
        const el = this.getElement(data);
        if (!el) {
            return;
        }
    ...
    public static getElement(data: string | Element): Element | undefined {
        const el = typeof data === 'string' ? this.instance.shadowRoot!.querySelector(`[data-id="${data}"]`) : data;
        if (!el) {
